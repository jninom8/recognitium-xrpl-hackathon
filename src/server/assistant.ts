...
export async function assist(input: AssistantInput, options: {key?:string; model?:string; transport?:typeof fetch}={}) {
  ...
  return {source:'mistral' as const,reply:result.reply,draft:assistantDraft(result.draft)};
}
