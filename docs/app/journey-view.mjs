...
export function proofCards(request, mode, operatorHref) {
  ...
  return `<section class="verification-section" aria-label="Verification evidence"><div class="section-heading"><h2>What backs this record?</h2><button class="text-button" data-open-page="receipts">View receipts</button></div><div class="proof-cards">${cards.map(([title, status, note, failed]) => `<article class="proof-card ${failed ? 'proof-failed' : ''}"><h3>${title}</h3><strong>${esc(status)}</strong><p>${esc(note)}</p></article>`).join('')}</div>${evidenceLimits()}</section>`;
}
...
export function simpleProofs(request) {
  ...
  const receiptIdentity=(r,label)=>r?'<p><strong>'+label+' receipt ID</strong><br><code class="hash">'+esc(r.receiptId)+'</code></p><p>Committed SHA-256<br><code class="hash">'+esc(r.commitmentHash)+'</code></p>':'';
  const receiptLink=(r,label)=>r&&/^DG-[a-f0-9]{32}$/i.test(r.receiptId)?'<a target="_blank" rel="noopener" href="https://www.recognitium.com/verify?id='+encodeURIComponent(r.receiptId)+'">'+label+' ↗</a>':'';
  return '<section class="simple-proofs" aria-label="XRPL and Recognitium evidence"><article><h3>XRPL</h3><p>'+ (funded?'Money transfer confirmed':'Money transfer not confirmed')+'</p><details><summary>See proof</summary><p>Saved ledger result'+(tx?.ledgerIndex?' · Ledger '+esc(tx.ledgerIndex):'')+'.</p>'+(funded&&/^[A-F0-9]{64}$/i.test(tx.hash)?'<a target="_blank" rel="noopener" href="https://custom.xrpl.org/lending-hackathon.dev.ripplex.io:51233/transactions/'+tx.hash+'">Open XRPL transaction ↗</a>':'')+'<p>A live check needs the event network.</p></details></article><article><h3 translate="no">Recognitium</h3><p>'+(request.checks?.contentHash==='mismatch'?'Agreement changed':failed?'Receipt check failed':checked?'Agreement receipt checked':'Agreement receipt pending')+'</p>'+receiptIdentity(receipt,'Agreement')+receiptIdentity(request.executionReceipt,'Payment')+'<details><summary>See proof</summary><p>Recognitium records the agreement fingerprint. It does not move the money.</p>'+receiptLink(receipt,'Open agreement receipt')+receiptLink(request.executionReceipt,'Open payment receipt')+'<p>Last agreement check: '+esc(checked?time(receipt.authorityCheckedAt):'Not recorded')+'</p><p class="hash">Agreement: '+esc(request.agreementHash)+'</p></details></article></section>';
}
