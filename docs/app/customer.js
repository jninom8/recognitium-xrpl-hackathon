import './pages-transport.js';
import {showReceiptRegister,showNetworkGate} from '/recognitium-xrpl-hackathon/app/receipt-register.js';
import { mountMarket, prepareAvailability, isMarketBusy } from '/recognitium-xrpl-hackathon/app/market.js';
import { financingJourney, exactApproval, duration } from '/recognitium-xrpl-hackathon/app/journey-model.mjs';
import { journeyPanel, proofCards, operationHistory, setupSummary, authorizationGuide, trackOneEvidence, simpleProofs } from '/recognitium-xrpl-hackathon/app/journey-view.mjs';
import { WalletReadings, walletPanel } from '/recognitium-xrpl-hackathon/app/wallet-panel.mjs';
import { createConversation } from '/recognitium-xrpl-hackathon/app/assistant.js';
const walletReadings = new WalletReadings();
import { SnapshotCursor, sameReview, drops, cycleFor, freshnessLabel } from "/recognitium-xrpl-hackathon/app/state-client.mjs";
import {
  ...
  customerLoan,
  restartDemoUrl,
} from "/recognitium-xrpl-hackathon/app/customer-model.mjs";
...
const pathView = {"/recognitium-xrpl-hackathon/borrow.html":"borrow", "/recognitium-xrpl-hackathon/review.html":"review", "/recognitium-xrpl-hackathon/lend.html":"lend"}[location.pathname];
...
if (role() === "broker") $("source").value = "recorded";
function enterWorkspace(selectedRole) {
  $("profile").value = selectedRole;
  document.body.classList.remove("welcome");
  const url = new URL(location.href);
  url.pathname = selectedRole === "lender" ? "/recognitium-xrpl-hackathon/lend.html" : selectedRole === 'broker' ? '/recognitium-xrpl-hackathon/review.html' : "/recognitium-xrpl-hackathon/borrow.html";
...
function chooseRequest(id) {
  ...
  const url = new URL(location.href); if(id){url.searchParams.set('request', id);url.searchParams.delete('new');}else{url.searchParams.delete('request');url.searchParams.set('new','1');} history.replaceState(null, '', url);
  closeReview(); render();
}
...
function render() {
  ...
  $('role-switch').href = relatedUrl(recorded ? lender ? '/recognitium-xrpl-hackathon/borrow.html' : '/recognitium-xrpl-hackathon/lend.html' : role() === 'broker' ? '/recognitium-xrpl-hackathon/borrow.html' : '/recognitium-xrpl-hackathon/review.html',id);
  ...
  $('profile').querySelector('option[value="broker"]').hidden = false;
  ...
  if (!recorded && !lender && hasAccess() && !privateAvailable) $('overview-content').innerHTML = empty(inboxLoaded ? 'Request connection paused.' : 'Opening the shared requests…',inboxLoaded ? 'Your saved requests are retained. Reconnect before making a decision.' : 'Reading the current request and review status.');
    ...
    const controls = r ? '<button class="primary" data-loan>View exact offer →</button>' : selectedIntake ? '<button class="primary" data-intake="'+esc(id)+'">'+(borrower?'View request':'Open review')+' →</button>' : borrower ? '<button class="primary" data-new>Request funding →</button>' : '<button class="primary" data-open-page="requests">Open requests →</button>';
    ...
    $('overview-content').innerHTML = journeyPanel(context,controls) + metrics + walletPanel(r,state.mode,walletReadings) + (r ? '<div class="dashboard-grid">'+loanCard(r,c,loanOutcome(r,c))+journeyCard(r,story)+'</div>'+proofCards(r,state.mode,'#receipts') : borrower && !selectedIntake ? borrowerStart() : '') + (!recorded && (!borrower || privateRequests().length) ? '<div class="section-heading"><h2>'+(!borrower?'Review inbox':'Shared requests')+'</h2><button class="text-button" data-open-page="requests">View all →</button></div>'+requestList(true) : '');
  }
  ...
  if(lender && r) $('overview-content').insertAdjacentHTML('beforeend',proofCards(r,state.mode,'#receipts'));
  ...
  if(role()==='broker'){
    ...
    identity.innerHTML='<h3>Identity + wallet</h3><p><strong>KYC commitment: not provided</strong></p><p class="hash">Wallet: '+esc(r?.agreement.accounts.borrower??'Not prepared for this request')+'</p><p>Wallet control: '+(r?'Backend-managed test account':'Not established')+'</p><p>Synthetic demo only. No KYC provider is connected.</p>'+(r?'<details><summary>Agreement document commitment (not KYC)</summary><p class="hash">'+esc(r.agreement.documentCommitment)+'</p></details>':'');
    const binding=r?.agreement.identity;
    if(binding)identity.innerHTML='<h3>Identity + wallet</h3><p><strong>Synthetic identity commitment included</strong></p><p>Bound to this request and borrower in the agreement. No real KYC performed.</p><p class="hash">Wallet: '+esc(binding.wallet)+'</p><details><summary>Identity commitment</summary><code class="hash">'+esc(binding.commitment)+'</code><p>Covered by the agreement hash. Receipt authority and XRPL confirmation remain separate checks.</p></details>';
    else identity.insertAdjacentHTML('beforeend','<details><summary>Try a synthetic identity example</summary><p>A fictional profile bound to a demo wallet. This preview does not modify this request or its existing receipts.</p><button type="button" id="identity-fixture-preview">Show test commitment</button><div id="identity-fixture-result"></div></details>');
    $('overview-content').querySelector('.journey-panel')?.after(identity);
    const preview=identity.querySelector('#identity-fixture-preview');if(preview)preview.onclick=async()=>{preview.disabled=true;const output=identity.querySelector('#identity-fixture-result');try{const response=await fetch('/recognitium-xrpl-hackathon/api/identity-fixture',{signal:AbortSignal.timeout(10000)});if(!response.ok)throw Error();const fixture=await response.json();output.innerHTML='<p><strong>Synthetic preview · not sealed</strong></p><p>Fictional supplier · no KYC provider</p><p class="hash">Example wallet: '+esc(fixture.opening.wallet)+'</p><p class="hash">Request: '+esc(fixture.opening.requestId)+'</p><code class="hash">'+esc(fixture.commitment)+'</code><p>'+esc(fixture.status)+'</p>'; }catch{output.textContent='Could not load the example. Try again.';}finally{preview.disabled=false;}};
  }
  ...
  if(role()==='broker' && state.mode==='live' && (r || selectedIntake?.status==='REVIEWED')) {
    ...
    if(state.hosting)void fetch('/recognitium-xrpl-hackathon/api/runner').then(x=>x.ok?x.json():null).then(x=>{if(!box.isConnected)return;box.querySelector('p').textContent=x?.connected?'Local runner connected. Exact approvals are checked again before execution.':'Local runner offline. Keep the operator computer running.';box.querySelector('button').disabled=!x?.connected||!ready||complete;}).catch(()=>{box.querySelector('p').textContent='Runner connection unavailable.';});
    box.querySelector('button').onclick=async()=>{
      const button=box.querySelector('button'),status=box.querySelector('#run-approved-status');button.disabled=true;
      try{const response=await fetch('/recognitium-xrpl-hackathon/api/requests/'+encodeURIComponent(id)+'/recognitium-xrpl-hackathon/app/run',{method:'POST',headers:{'Content-Type':'application/json',...(access.token?{Authorization:'Bearer '+access.token}:{})},body:JSON.stringify({agreementHash:r.agreementHash,transactionDigest:r.transactionDigest}),signal:AbortSignal.timeout(15000)});
  ...
  if(state.mode==='live' && !state.hosting && id) {
    ...
    void fetch('/recognitium-xrpl-hackathon/api/automatic').then(r=>r.ok?r.json():null).then(job=>{
...
function borrowerStart() {
  return '<div class="help-grid"><section class="card help-card"><span class="feature-icon" aria-hidden="true">↗</span><div><h3>One request, two perspectives</h3><p>You ask for funding. A reviewer checks the request. Both views follow the same record.</p><a class="text-button" href="/recognitium-xrpl-hackathon/review.html" target="_blank" rel="noopener">Explore the reviewer workspace ↗</a></div></section><section class="card help-card"><span class="feature-icon" aria-hidden="true">✓</span><div><h3>Understand before you commit</h3><p>The local operator prepares the exact offer and handles test transactions after explicit approval. The progress appears here.</p><a class="text-button" href="/recognitium-xrpl-hackathon/index.html?mode=recorded">Explore a completed example →</a></div></section></div>';
}
...
function renderLender(r, c) {
  ...
  if (!deposited) {
    $("overview-content").innerHTML = '<details class="card"><summary>See a completed loan</summary><p>The recorded example shows a real test-network deposit, repayment and withdrawal.</p><a href="/recognitium-xrpl-hackathon/app/lend?mode=recorded">Open evidence example →</a></details>';
    return;
  }
...
function activity(r,c) {
  ...
  const intakeHistory = intake ? '<section class="card"><p class="eyebrow">REQUEST & REVIEW</p><h2>Decisions kept in order.</h2><ol class="operation-history">'+intake.history.map(e=>'<li><span class="operation-dot">✓</span><div><h3>'+esc({'submitted':'Request submitted','start-review':'Reviewer started checking','request-revision':'Revision requested','finish-review':'Review completed','reject-request':'Request declined'}[e.event])+'</h3><p>'+esc(fullTime(e.at))+' · '+esc(e.role)+'</p></div></li>').join('')+'</ol></section>' : '';
  return intakeHistory + trackOneEvidence(r,c) + '<div class="section-heading"><h2>Money movement & preparation</h2></div><section class="card">'+setupSummary(c)+operationHistory(r,c,state.network.explorer)+'</section>' + (r ? proofCards(r,state.mode,'#receipts')+'<div class="record-links"><section class="card"><h3>Exact agreement</h3><p class="hint">Version '+r.agreement.documentVersion+'. All rates, fees and signature commitments remain available.</p><button class="text-button" data-loan>View agreement →</button></section><section class="card"><h3>Keep the evidence</h3><p class="hint">'+(state.mode==='recorded'?'Download the synthetic bundle for independent checks.':'This request’s private export is prepared by the local operator. The completed example has its own separate bundle.')+'</p>'+(state.mode==='recorded'?'<a class="text-button" href="/recognitium-xrpl-hackathon/api/evidence/published" download="synthetic-supplier-001.json">Download example evidence ↓</a>':'<button class="text-button" data-open-page="receipts">View receipts</button>')+'</section></div>':'') + '<p class="evidence-note">A document match, an authority receipt and a ledger result answer different questions. This history keeps them separate. Records shown here are saved observations; a page refresh is not an online verification.</p>';
}
async function refresh() {
  const ticket = cursor.begin($("source").value);
  try {
    const response = await fetch("/recognitium-xrpl-hackathon/api/state?mode=" + $("source").value, {
...
async function loadIntake() {
  ...
  try {
    const response = await fetch("/recognitium-xrpl-hackathon/api/intake?role=" + session.role, {
...
$("access-form").addEventListener("submit", async (e) => {
  ...
  try {
    const response = await fetch("/recognitium-xrpl-hackathon/api/intake?role=" + selectedRole, {
...
function showDraftReview() {
  ...
  $("request-intro").textContent =
    "Save these details to propose a lender match. You are not agreeing to borrow money yet.";
...
$("request-form").addEventListener("submit", async (e) => {
  ...
  try {
    const response = await fetch("/recognitium-xrpl-hackathon/api/intake", {
      method: "POST",
      headers: {
    ...
    const url = new URL(location.href); url.searchParams.set('request', selectedLoanId); url.searchParams.delete('new'); history.replaceState(null, '', url);
    clearPending();
    draft = undefined;
    ...
    setPage("overview");
    notice(
      "Request saved. Propose a lender match below, then approve it for review. No money moved.",
    );
  } catch (error) {
    requestError(
      error.message +
...
function openIntake(id) {
      ...
      ? request.status === "AWAITING_REVIEW"
        ...
        : request.status === "UNDER_REVIEW"
          ? [
              ...
              ["finish-review", "Accept request"],
              ["reject-request", "Decline request"],
            ]
          : []
      : [];
  $("review-actions").innerHTML =
    decisions
      .map(
        ([decision, label]) =>
          `<button class='${decision !== "finish-review" && decision !== "start-review" ? "secondary" : "primary"}' data-decision='${decision}'>${label}</button>`,
      )
      .join("") || `<button class='secondary' data-close-review>Done</button>`;
  if (role() === "broker")
    $("review-content").innerHTML +=
      `<p class='hint'>This is an intake decision. Loan signing still needs separate approval of the exact terms.</p>`;
...
async function applyReview(decision) {
  ...
  const path =
    current.kind === "intake"
      ? "/recognitium-xrpl-hackathon/api/intake/" + current.request.clientRequestId + "/recognitium-xrpl-hackathon/review.html"
      : "/recognitium-xrpl-hackathon/api/requests/" + current.requestId + "/recognitium-xrpl-hackathon/app/approve/" + role();
  const input =
    current.kind === "intake"
      ? {
...
document.addEventListener("click", (e) => {
  ...
  if(b.dataset.intention){
    ...
    const url=new URL(location.href);url.pathname=b.dataset.intention==='broker'?'/recognitium-xrpl-hackathon/review.html':b.dataset.intention==='lender'?'/recognitium-xrpl-hackathon/lend.html':'/recognitium-xrpl-hackathon/borrow.html';history.replaceState(null,'',url);
