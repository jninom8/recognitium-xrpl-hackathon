...
## September 13: one frontend, evidence retained

The interface audit removed the obsolete operator-console links and duplicate footer menus, while retaining Saved rounds and in-page Transactions & evidence. A real shared-backend synthetic request (6AB91026, 101.000001 test XRP) was submitted, reviewed and declined through Chrome; the borrower saw the refusal and Reset retained the record. This was an application refusal, not a native XRPL rejection. No funds moved. Matching display now preserves six decimal places. Receipt cards explain discovery, bilateral matching, agreement and execution separately; no claim that every transition already has a receipt.
