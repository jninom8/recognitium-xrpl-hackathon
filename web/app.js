...
import { financingJourney, exactApproval } from '/journey-model.mjs';
import { journeyPanel, operationHistory, setupSummary, proofCards, authorizationGuide } from '/journey-view.mjs';
...
function selected() { return selectedId ? state?.requests.find(r => r.agreement.requestId === selectedId) : state?.requests[0]; }
function syncText(snapshot) {
  return freshnessLabel(snapshot, selectedId);
}
...
function render() {
  ...
  const r = selected(); selectedId ??= r?.agreement.requestId ?? state.cycle?.requestId;
  renderedExpiryKey = expiryKey(state);
  const recorded = state.mode === 'recorded';
  $('check-health').hidden = Boolean(state.hosting);
  const bridge = state.bridgeByRequest?.[selectedId];
  ...
  $('sync-status').textContent = available ? syncText(state) : 'Backend unavailable. Last displayed facts are retained; actions paused.';
  ...
  const c = cycleFor(state, selectedId);
  const story = financingJourney({request:r, intake:selectedIntake, cycle:c, bridge, mode:state.mode});
  const linkQuery = new URLSearchParams({mode:state.mode,...(selectedId?{request:selectedId}:{})});
  $('admin-journey').innerHTML = journeyPanel({request:r,intake:selectedIntake,cycle:c,bridge,mode:state.mode},`<a class="secondary" href="${recorded?'/borrow':'/review'}?${escape(linkQuery)}">Open ${recorded?'borrower':'reviewer'} workspace ↗</a>`);
  ...
  $('money-title').textContent = r?.funding.status === 'funded' ? money : story.title;
  ...
  if (!r) {
    $('agreement-panel').innerHTML = '<span class="eyebrow">THE COMMITMENT</span><h2>No exact offer recorded here yet.</h2><p class="position-note">Complete the intake review, then use the local bridge to prepare this request’s offer. Viewing or reviewing intake does not approve a loan.</p>';
    $('execution-panel').innerHTML = '<span class="eyebrow">THE EXECUTION</span><h2>No loan transaction recorded.</h2><p class="position-note">Ledger operations for this request will appear here as the local operator publishes progress.</p>';
  } else {
    const a = r.agreement, t = a.terms;
    $('agreement-panel').innerHTML = `<div class="panel-heading"><div><span class="eyebrow">${escape(a.requestId)}</span><h2>Supplier agreement</h2></div>${badge(`Version ${a.documentVersion}`)}</div><div class="principal"><span>Loan principal</span><p>${escape(drops(t.principalDrops))} <span>test XRP</span></p></div><dl class="terms"><div><dt>Annual interest</dt><dd>${escape(t.interestRate / 1000)}%</dd></div><div><dt>Schedule</dt><dd>${escape(t.paymentTotal)} payment · ${escape(t.paymentInterval)} seconds</dd></div><div><dt>Approval expiry</dt><dd>${escape(time(a.expiresAt))}</dd></div><div><dt>Transaction expiry</dt><dd>Ledger ${escape(r.preparedTransaction.LastLedgerSequence)}</dd></div></dl><p class="small-print">${recorded ? 'The public bundle verifies both transaction signatures. Human approval timestamps were not exported.' : 'Each role approves the exact hashes below. Expired terms cannot be approved. Preserve this run while resolving renewal.'}</p>${['broker','borrower'].map(role => `<div class="approval-row"><span>${role === 'broker' ? 'Broker' : 'Borrower'}</span><span>${recorded ? 'Signature verified offline' : exactApproval(r,role) ? 'Exact approval recorded' : 'Approval required'}</span></div>`).join('')}<div class="action-grid">${actionButtons(r.actions.filter(x => x.id.startsWith('approve/')), a.requestId)}</div><details><summary>Exact terms, accounts and signature binding</summary>${json({agreement:a,agreementHash:r.agreementHash,transactionDigest:r.transactionDigest,preparedTransaction:r.preparedTransaction,approvals:r.approvals})}</details>`;
    $('execution-panel').innerHTML = `<span class="eyebrow">THE EXECUTION</span><h2>Preparation to lender return.</h2>${setupSummary(c)}${operationHistory(r,c,state.network.explorer)}<p class="small-print">${escape(r.phase)}. Backend-managed demo accounts. No browser wallet extension is connected.</p>`;
  }
  ...
  if (!r && c) $('execution-panel').innerHTML = '<h2>Offer preparation</h2>'+setupSummary(c)+operationHistory(undefined,c,state.network.explorer);
  $('operator-scope').textContent = state.hosting ? 'This site publishes progress. Setup, exact approvals, signing, submission, receipt recovery, repayment and withdrawal run through the local operator bridge. No signing keys are deployed here.' : 'These controls act on this local backend. Human approval binds the exact agreement and transaction. Receipt recovery preserves the original funding.';
  $('admin-wallet').innerHTML=walletPanel(r,state.mode,walletReadings);
  $('admin-authorization').innerHTML=authorizationGuide('admin',Boolean(state.hosting));
  $('proof-summary').innerHTML = proofCards(r,state.mode,'#proof');
  $('proof').innerHTML = '<details><summary>Stored verification details</summary>'+$('proof').innerHTML+'</details>';
  if (r) $('proof').innerHTML += '<details><summary>Receipt identifiers, commitments and recovery state</summary>'+json({agreementReceipt:r.agreementReceipt,executionReceipt:r.executionReceipt,recovery:r.receiptRecovery,checks:r.checks})+'</details>';
  $('published-download').textContent = recorded ? 'Download this completed example’s evidence bundle' : 'Download the original completed example (a different run)';
  $('export-note').textContent = recorded ? 'The bundle checks fingerprints, signatures and receipt consistency offline. Authenticating the receipt source and rechecking XRPL inclusion need the online verifier. No complete offline ledger proof is included.' : 'A fresh request export must be produced from its isolated run directory by the local operator. This link never exports the selected live request.';
...
async function refresh() {
  ...
  try {
    ...
    let nextIntake;
    if(next.hosting?.openDemo && next.mode==='live' && selectedId) {
      try { const inbox=await fetch('/api/intake?role=broker',{signal:AbortSignal.timeout(10000)});if(inbox.ok)nextIntake=(await inbox.json()).requests.find(r=>r.clientRequestId===selectedId); } catch { /* No intake claim if its read fails. Native facts remain independently available. */ }
    }
    if(!cursor.accept(ticket,next))return;
    const changed=!available || !state || state.instanceId!==next.instanceId || state.revision!==next.revision || state.mode!==next.mode || renderedExpiryKey!==expiryKey(next) || JSON.stringify(selectedIntake)!==JSON.stringify(nextIntake);
    selectedIntake=nextIntake;
    available=true;state=next;if(changed)render();else $('sync-status').textContent=syncText(next);
  } catch {
...
document.addEventListener('click',e=>{if(e.target.closest('[data-balance]')){const r=selected();if(r){const work=walletReadings.read(r,state.mode);render();void work.then(()=>render());}}});
...
$('mode').addEventListener('change',()=>{closeReview();selectedId=undefined;selectedIntake=undefined;message('');const url=new URL(location.href);url.searchParams.set('mode',$('mode').value);url.searchParams.delete('request');history.replaceState(null,'',url);void refresh();});
$('request-picker').addEventListener('change',e=>{selectedId=e.target.value;selectedIntake=undefined;const url=new URL(location.href);url.searchParams.set('request',selectedId);history.replaceState(null,'',url);render();void refresh();});
function showView(view){for(const v of ['request','position','evidence'])$(`${v}-view`).hidden=view!==v;document.querySelectorAll('[data-view]').forEach(q=>q.setAttribute('aria-pressed',String(q.dataset.view===view)));}
document.querySelectorAll('[data-view]').forEach(b=>b.addEventListener('click',()=>{showView(b.dataset.view);const url=new URL(location.href);url.searchParams.set('tab',b.dataset.view);history.replaceState(null,'',url);}));
const initialTab=new URL(location.href).searchParams.get('tab');if(['request','position','evidence'].includes(initialTab))showView(initialTab);
